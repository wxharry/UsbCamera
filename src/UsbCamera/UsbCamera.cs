...
namespace UsbCamera.Net
{
    ...
    public class UsbCamera
    {
        ...
        private class SampleGrabberBuffer
        {
            ...
            {
                // To retrieve an image from the Sample Grabber:
                // 1) Call GetCurrentBuffer with size=0 to obtain required buffer size.
                // 2) Allocate the buffer and call GetCurrentBuffer again to fill it.
                // Captured images are bottom-up; inversion (vertical flip) may be required depending on consumer.
                int size = 0;
                Grabber.GetCurrentBuffer(ref size, IntPtr.Zero); // First call with IntPtr.Zero obtains buffer size
                if (size == 0) return null;

                if (Buffer == null || size != Buffer.Length)
                {
                    // Allocate unmanaged memory for buffer
                ...
                // Retrieve image data into the unmanaged buffer and copy to managed byte[]
                Grabber.GetCurrentBuffer(ref size, BufPtr);

                Marshal.Copy(BufPtr, Buffer, 0, size);

                // Build bitmap from captured buffer
        ...
        /// <summary>
        /// Creates the Sample Grabber filter.
        /// </summary>
        private DirectShow.IBaseFilter CreateSampleGrabber(VideoFormat format)
        {
            ...
            // When first created the Sample Grabber has no preferred media type.
            // That means it can connect to almost any upstream filter but cannot constrain the data type.
            // Therefore call ISampleGrabber::SetMediaType before building the rest of the graph to define required media characteristics.
            // The Sample Grabber compares the source media type against the specified MajorType, SubType, and FormatType.
            // GUID_NULL in these fields means "accept any". Typically specify both MajorType and SubType.
            // The filter rejects top-down (negative biHeight) video types and FORMAT_VideoInfo2 types.
            // References:
            // https://learn.microsoft.com/windows/win32/directshow/sample-grabber-filter
            // (Original Japanese MSDN links replaced by English documentation.)

            var mt = new DirectShow.AM_MEDIA_TYPE();
            mt.MajorType = DirectShow.DsGuid.MEDIATYPE_Video;

            // By setting the media type early you can still leverage Intelligent Connect.
            // For example, specifying uncompressed video will cause the graph builder to insert any required parsers/decoders automatically.

            // For Y8 / Y800 / Y16 do not convert; receive raw samples. Otherwise request RGB24 conversion.
        ...
        /// <summary>
        /// Creates the Video Capture Source filter instance.
        /// </summary>
        ...
        /// <summary>
        /// Selects the video capture device output format.
        /// </summary>
        private static void SetVideoOutputFormat(DirectShow.IPin pin, VideoFormat format)
        {
            var formats = GetVideoOutputFormat(pin);

            // Specification: a VideoCaptureDevice may expose a range of output formats per media type (example):
            // [0]: YUY2 Min:160x120 Max:320x240 Horizontal step:4 Vertical step:2
            // [1]: RGB8 Min:640x480 Max:640x480 (fixed size)
            // SetFormat can adjust output size and frame rate within the supported range.
            // In practice many consumer USB cameras report fixed sizes (min == max).

            // Most VIDEO_STREAM_CONFIG_CAPS members are deprecated; prefer IAMStreamConfig::GetFormat.
            // Relevant members: Guid (FORMAT_VideoInfo / FORMAT_VideoInfo2), VideoStandard (NTSC/PAL via AnalogVideoStandard),
            // MinFrameInterval / MaxFrameInterval (frame duration in 100ns units).

            // Devices following current guidance often return fixed output sizes; older devices may offer variable ranges.
            // Outline below describes how resolution, cropping and frame rate can be adjusted using VIDEO_STREAM_CONFIG_CAPS.

            // (1) Frame rate:
            // MinFrameInterval / MaxFrameInterval define min/max frame durations.
            // fps = 10,000,000 / frameDuration (in 100ns units)

            // To request a specific frame rate set VIDEOINFOHEADER / VIDEOINFOHEADER2 AvgTimePerFrame; driver will choose nearest supported value.

            // (2) Cropping example:
            // MinCroppingSize = (160,120)  // Minimum crop size
            // MaxCroppingSize = (320,240)  // Maximum crop size
            // CropGranularityX = 4         // Horizontal increment
            // CropGranularityY = 8         // Vertical increment
            // CropAlignX = 2               // Alignment constraint for left edge
            // CropAlignY = 4               // Alignment constraint for top edge

            // (3) Output size:
            // VIDEO_STREAM_CONFIG_CAPS gives min/max width & height and step granularity for valid increments.
            // Example values:
            ...
            // Permitted widths: 160,168,176,...,304,312,320; heights: 120,128,136,...,224,232,240.

            // Uncomment below for debugging variable output size (no such camera available to author):
            ...
            // First attempt to match an exact fixed size (VIDEO_STREAM_CONFIG_CAPS mostly deprecated).
            // VIDEO_STREAM_CONFIG_CAPS is deprecated. First, find just the fixed size.
            for (int i = 0; i < formats.Length; i++)
            {
                var item = formats[i];

                // Accept only VideoInfo (SampleGrabber does not support VideoInfo2)
            ...
            // If no fixed size found, search variable size range.
            ...
            // If still not found, fall back to default size.
        ...
        /// <summary>
        /// Enumerates supported media types and sizes for the video capture device.
        /// </summary>
        private static VideoFormat[] GetVideoOutputFormat(DirectShow.IPin pin)
        {
            // Acquire IAMStreamConfig interface
            ...
            // Allocate unmanaged capability buffer
            var cap_data = Marshal.AllocHGlobal(cap_size);

            // Enumerate each capability entry
            for (int i = 0; i < cap_count; i++)
            {
                var entry = new VideoFormat();

                // Retrieve the i-th format capability
        ...
        /// <summary>
        /// Selects the device output format by index (obtained via GetVideoOutputFormat) optionally adjusting size and frame duration.
        /// </summary>
        /// <param name="index">Desired format index.</param>
        /// <param name="size">If not Empty, overrides output size within supported range.</param>
        /// <param name="timePerFrame">If >0, overrides AvgTimePerFrame (frame duration).</param>
        private static void SetVideoOutputFormat(DirectShow.IPin pin, int index, Size size, long timePerFrame)
        {
            ...
            // Retrieve selected format information
            DirectShow.AM_MEDIA_TYPE mt = null;
        ...
        {
            public string MajorType { get; set; }  // e.g. "Video"
            public string SubType { get; set; }    // e.g. "YUY2", "MJPG"
            public Size Size { get; set; }         // Frame dimensions
            public long TimePerFrame { get; set; } // Average frame duration in 100ns units (e.g. 333333 for 30fps)
...
#region Function
        ...
        /// <summary>Change filter graph state (run/pause/stop).</summary>
        ...
        /// <summary>Get list of filters in a device category.</summary>
        ...
        /// <summary>Create filter instance by CLSID.</summary>
        public static IBaseFilter CreateFilter(Guid clsid)
        {
            return CoCreateInstance(clsid) as IBaseFilter;
        }

        /// <summary>Create filter instance by category and index.</summary>
        public static IBaseFilter CreateFilter(Guid category, int index)
        {
            ...
            EnumMonikers(category, (moniker, prop) =>
            {
                // Continue until desired index reached.
                if (index != curr_index++) return false;

                // Create filter instance and return.
                {
                    object value = null;
        ...
        /// <summary>Enumerate monikers in given category.</summary>
        /// <remarks>A moniker is a COM object binding name.</remarks>
        private static void EnumMonikers(Guid category, Func<IMoniker, IPropertyBag, bool> func)
        {
            ...
            try
            {
                ...
                while (enumerator.Next(monikers.Length, monikers, fetched) == 0)
                {
                    ...
                    {
                        // Release property bag.
                        Marshal.ReleaseComObject(prop);

                        // Release enumerated moniker.
        ...
        /// <summary>Find pin by name.</summary>
        ...
        /// <summary>Find pin by output/input index and direction.</summary>
        ...
        /// <summary>Find pin by index, direction and category.</summary>
        ...
        /// <summary>Enumerate pins on a filter.</summary>
        ...
        /// <summary>Connect two pins (output to input).</summary>
        ...
        /// <summary>Free an AM_MEDIA_TYPE.</summary>
